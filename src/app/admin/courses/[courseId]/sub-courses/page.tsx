...
import { fetchCourses, createSubCourse, updateSubCourse, deleteSubCourse, AdminSubCourse, AdminCourse } from '@/lib/redux/slices/coursesSlice'
...
export default function CourseSubCoursesPage() {
  ...
  const subCourses = useMemo(() => course?.sub_courses || [], [course])
  ...
  const pageTitle = useMemo(() => {
    if (!course) return 'Manage Sub-Courses'
    return `Manage Sub-Courses • ${course.title}`
  }, [course])
  ...
  const validateSubCourseForm = (): { title: string; orderNumber: number; finalDescription: string | null } | null => {
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    try {
      ...
      const validated = validateSubCourseForm()
      if (!validated) { setIsCreating(false); return }
      ...
      await dispatch(createSubCourse({
      ...
      setLocalError(getErrorMessage(err) || 'Failed to create sub-course')
    } finally {
      setIsCreating(false)
    }
  }
  ...
  const openEditModal = (item: AdminSubCourse) => {
    setLocalError('')
    setFormConfig({
      title: item.title,
  ...
  const handleEdit = async (e: FormEvent) => {
    ...
    try {
      ...
      await dispatch(updateSubCourse({
        id: editingId,
        data: {
          title: title,
          description: finalDescription,
      ...
      setLocalError(getErrorMessage(err) || 'Failed to update sub-course')
    } finally {
      setIsCreating(false)
    }
  }

  const handleDelete = async (item: AdminSubCourse) => {
    const confirmed = window.confirm(`Delete module "${item.title}"? All exams inside will also be deleted.`)
    if (!confirmed) return
    ...
    try {
      ...
      await dispatch(deleteSubCourse(item.id)).unwrap()
    } catch (err: any) {
      setLocalError(getErrorMessage(err) || 'Failed to delete sub-course')
    }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900">Sub-Courses / Modules</h2>
          <button
        ...
        <section className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
          {loadingData ? (
            ...
            <div className="text-center py-12">
               <p className="text-gray-500 mb-4">No sub-courses yet in this course</p>
               <button onClick={openCreateModal} className="px-4 py-2 bg-gray-900 text-white rounded-lg hover:bg-black">
                 Create your first Sub-Course
               </button>
            </div>
          ) : (
                ...
                <div key={item.id} className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow">
                  ...
                  <div className="p-4">
                    ...
                    <div className="space-y-2">
                      <Link 
                        href={`/admin/courses/${courseId}/sub-courses/${item.id}/exams`}
                      ...
                      <div className="flex gap-2">
                        <button className="flex-1 px-2 py-2 text-xs rounded-md bg-gray-100 text-gray-700 hover:bg-gray-200" onClick={() => openEditModal(item)}>Edit</button>
                        <button className="flex-1 px-2 py-2 text-xs rounded-md bg-red-100 text-red-700 hover:bg-red-200" onClick={() => handleDelete(item)}>Delete</button>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
              <h2 className="text-xl font-semibold text-gray-900">Create Sub-Course</h2>
              <button onClick={closeCreateModal} className="text-gray-500 hover:text-gray-700">✕</button>
            </div>
            <form onSubmit={handleCreate} className="p-6 space-y-4">
              {error && <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg text-sm">{error}</div>}

              <input className="w-full border border-gray-300 rounded-lg px-3 py-2" placeholder="Sub-course module title" value={formConfig.title} onChange={(e) => setFormConfig({...formConfig, title: e.target.value})} required />
              <textarea className="w-full border border-gray-300 rounded-lg px-3 py-2" placeholder="Description (optional)" value={formConfig.description} onChange={(e) => setFormConfig({...formConfig, description: e.target.value})} rows={3} />
              
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-2 col-span-2">
                  <label className="block text-sm font-medium text-gray-700">Order Number</label>
                  <input type="number" min={0} className="w-full border border-gray-300 rounded-lg px-3 py-2" value={formConfig.order_number} onChange={(e) => setFormConfig({...formConfig, order_number: e.target.value})} />
                </div>
              </div>

              <div className="space-y-2">
                <label className="block text-sm font-medium text-gray-700">Image (Optional)</label>
                <input type="file" accept="image/*" className="w-full border border-gray-300 rounded-lg px-3 py-2" onChange={(e) => setImageFile(e.target.files?.[0] || null)} />
              </div>
              
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={formConfig.is_active} onChange={(e) => setFormConfig({...formConfig, is_active: e.target.checked})} className="rounded text-teal-600" />
                <span className="text-sm text-gray-700">Active (Visible to students)</span>
              </label>

              <div className="flex gap-3 pt-4 border-t border-gray-200">
                <button type="button" onClick={closeCreateModal} className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">Cancel</button>
                <button type="submit" disabled={isCreating} className="flex-1 px-4 py-2 bg-gray-900 text-white rounded-lg hover:bg-black disabled:opacity-50">
                  {isCreating ? 'Creating...' : 'Create Sub-Course'}
                </button>
              </div>
            </form>
          </div>
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
              <h2 className="text-xl font-semibold text-gray-900">Edit Sub-Course</h2>
            ...
            <form onSubmit={handleEdit} className="p-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                {formConfig.image_url && !imageFile && !removeImage && (
                  <div className="flex items-center justify-between mb-3 p-2 border border-gray-200 rounded">
                    <img src={formConfig.image_url} alt="Current" className="h-12 w-16 object-cover rounded" />
                    <button type="button" onClick={() => setRemoveImage(true)} className="text-xs text-red-600 font-medium whitespace-nowrap">Remove</button>
                  </div>
                )}
                {removeImage && (
                  <div className="flex items-center justify-between p-2 bg-red-50 border border-red-200 rounded mb-3">
                    <span className="text-xs text-red-700">Image will be removed</span>
              ...
              <div className="flex gap-3 pt-4 border-t border-gray-200">
                <button type="button" onClick={closeEditModal} className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">Cancel</button>
                <button type="submit" disabled={isCreating} className="flex-1 px-4 py-2 bg-gray-900 text-white rounded-lg hover:bg-black disabled:opacity-50">
                  {isCreating ? 'Updating...' : 'Update Sub-Course'}
                </button>
              </div>
            </form>
          </div>
